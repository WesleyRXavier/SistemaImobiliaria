$(function () {
    ...
    $(".mobile_menu").click(function (event) {
        ...
        $(".dash_sidebar")
            .animate({ left: "0" }, 200)
            .addClass("mobile_menu_open");
        $("body").addClass("mobile_body");

        $("html").on("click touchstart", ".mobile_body", function (e) {
            var menu = $(".dash_sidebar");
            if (
                menu.hasClass("mobile_menu_open") &&
                !$(e.target).hasClass("mobile_menu")
            ) {
                if (
                    !$(e.target).hasClass("dash_sidebar") &&
                    !$(e.target).parents().hasClass("dash_sidebar")
                ) {
                    menu.animate({ left: "-260" }, 200).removeClass(
                        "mobile_menu_open"
                    );
                }
            }
    ...
    $(".search_open").click(function (event) {
        ...
        $(".dash_content_search")
            .animate({ right: "0" }, 200)
            .addClass("search_open");
        $("body").addClass("search");

        $("html").on("click touchstart", ".search", function (e) {
            var search = $(".dash_content_search");
            if (
                search.hasClass("search_open") &&
                !$(e.target).hasClass("search_open")
            ) {
                if (
                    !$(e.target).hasClass("dash_content_search") &&
                    !$(e.target).parents().hasClass("dash_content_search")
                ) {
                    search
                        .animate({ right: "-320px" }, 200)
    ...
    $(".collapse").click(function (event) {
        ...
        $(collapse)
            .find(".app_collapse_header > span")
            .toggleClass("icon-plus-circle")
            .toggleClass("icon-minus-circle");
        $(collapse)
            .find(".app_collapse_content")
    ...
    function normalizeSpouse() {
        if (typeof $('select[name="civil_status"]') !== "undefined") {//verifica se o campo existe
            if (
                $('select[name="civil_status"]').val() === "married" ||
                $('select[name="civil_status"]').val() === "separatted"
            ) {
                $(".content_spouse input, .content_spouse select").prop(
                    "disabled",
                    false
                );
            } else {
...
tinyMCE.init({
    ...
    toolbar:
        "styleselect | pastetext | removeformat |  bold | italic | underline | strikethrough | bullist | numlist | alignleft | aligncenter | alignright |  link | unlink | code | fullscreen",
    style_formats: [
